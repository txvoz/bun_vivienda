...
import { CreditDetailResponse, Participant } from 'src/app/interfaces/CreditDetailResponse';
...
export class CreditDetailMainComponent implements OnInit {
  ...
  loadData(){
    ...
    this.creditDetailService.getDetail(this.id).subscribe(data => {
      ...
      this.index = 0;
      this.pivote = this.datasource?.response?.participants?this.datasource?.response?.participants[this.index]:null;
  ...
  submit(){
    ...
    this.sharedDataService.changeStatus(true);
    this.interviewService.send(this.id, this.getInterviewRequest()).subscribe(data => {
      this.sharedDataService.changeStatus(false);
    });
  ...
  editRequest(edit:boolean = false) {
    this.router.navigateByUrl('/credit/request/' + this.id + (edit? "?option=edit" : "")); 
  }
